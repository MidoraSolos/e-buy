...
const Cart = () => {
	...
	useEffect(() => {
		const fetchCart = async () => {
			try {
				const cartId = 3; // Assuming you're working with cart ID 3 for now
					...
					<>
						...
						{cart.cartProducts.map((cartProduct) => (
							<div key={cartProduct.product.id} className="cart-item">
								<h2 className="product-name">{cartProduct.product.name}</h2>
								<h4 className="product-quantity">Quantity: 1</h4>
								<h2 className="product-price">
									Product Price: ${cartProduct.product.price}
								</h2>
								<button
									onClick={() => removeProduct(cartProduct.product.id)}
